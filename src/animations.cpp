		...
		for (j = 0; j < 3; j++)
			for (k = 0; k < 3; k++)
				...
				case Shake:
					...
					if (squeezes[i][j][k])
						rubiksCube->scaleCubelet(i, j, k, sin(animIndex * 2 * M_PI / 52.0f) * vec3(1 - 10 / shakeMag[i][j][k].x, 1 - 10 / shakeMag[i][j][k].y, 1 - 10 / shakeMag[i][j][k].z) + vec3(1.0f));
					break;
				case Rotate:
		...
		for (j = 0; j < 3; j++)
			for (k = 0; k < 3; k++)
				...
				case Shake:
					...
					if (squeezes[i][j][k])
						rubiksCube->scaleCubelet(i, j, k, sin(animIndex * 2 * M_PI / 200.0f) * vec3(1 - 10 / shakeMag[i][j][k].x, 1 - 10 / shakeMag[i][j][k].y, 1 - 10 / shakeMag[i][j][k].z) + vec3(1.0f));
					break;
				case Rotate:
